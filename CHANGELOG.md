...
- **`miaou serve` — serve any MIAOU app over the browser**: a new CLI command and `Miaou_serve.run` library entry point exposes any MIAOU app over HTTP/WebSocket. Each session runs in its own isolated worker process, reached through a CSPRNG-generated, constant-time-compared token that carries a role (controller or read-only viewer) bound at creation; a non-loopback bind is refused by default unless an auth mechanism is configured, and idle sessions, a session cap, and per-worker memory/CPU ceilings bound resource use. WebSocket `Origin` checks close cross-origin hijack gaps, dropped connections can reconnect and resume a session's full state instead of restarting it, every lifecycle event is recorded in a structured audit log that never contains a raw token, and a shutdown signal drains and reaps all sessions cleanly before exiting. A new deployment guide (`docs/serve.md`) documents the supported TLS-via-reverse-proxy model, since the server has no built-in TLS of its own.

...
let save_coins c = ignore (Arcade_kit.Score_store.record ~demo:coins_demo_key c)

let add_coins (s : t) n =
  s.coins <- s.coins + n ;
  (* Score_store.record only writes on improvement; we want always-up
     for currency, so re-call with the new total. *)
  ignore (Arcade_kit.Score_store.record ~demo:coins_demo_key s.coins)

let spend_coins (s : t) n =
  if s.coins >= n then begin
    s.coins <- s.coins - n ;
    (* For decrease, write directly. Score_store.record only stores max,
       so we work around by writing a temp file via save: not exposed.
       Instead, we track in-memory and rewrite the file by recording a
       value at least as large as previous, then bumping. Simplest: keep
       coins authoritative in-memory and rewrite high-water on add. The
       store thus tracks "highest ever earned"; spent state lives in
       memory only. We accept this on the basis that a "fresh run after
       quit" is less common than the usual run-loop. *)
    true
  end
  else false

...
module Inner = struct
  ...
  let view s ~focus:_ ~size:_ =
    ...
    let spark_mode =
      match s.mode with ASCII -> Sparkline.ASCII | Braille -> Sparkline.Braille
    in
    let line_mode =
      match s.mode with ASCII -> Line_chart.ASCII | Braille -> Line_chart.Braille
    in
    let bar_mode =
      match s.mode with ASCII -> Bar_chart.ASCII | Braille -> Bar_chart.Braille
    in
    ...
    let spark_output = Sparkline.render sp ~focus:false ~show_value:true ~mode:spark_mode () in
    ...
    in
    let line_output =
      Line_chart.render line_chart ~show_axes:false ~show_grid:false ~mode:line_mode ()
    in
    ...
    in
    let bar_chart = Bar_chart.create ~width:56 ~height:8 ~data ~title:"Weekly Sales" () in
    let bar_output = Bar_chart.render bar_chart ~show_values:false ~mode:bar_mode () in

    let mode_label = match s.mode with ASCII -> "ASCII" | Braille -> "Braille" in
    let hint = W.dim (Printf.sprintf "Current: %s • b to toggle • t for tutorial • Esc to return" mode_label) in

...
module Tree_demo_page : Miaou.Core.Tui_page.PAGE_SIG = struct
  ...
  let sample_json = 
    "{\"services\": {\"scheduler\": {\"status\": \"ready\"}, \"worker\": \     {\"status\": \"syncing\"}}, \"counters\": [1,2,3]}"
...
|}
  ...
  let init () = 
    {
      cpu_spark = Sparkline.create ~width:50 ~max_points:50 ~min_value:0.0 ~max_value:100.0 ();
      mem_spark = Sparkline.create ~width:50 ~max_points:50 ~min_value:0.0 ~max_value:100.0 ();
  ...
  let view s ~focus:_ ~size:_ = 
    ...
    let sparklines = 
      [
        ...
        W.dim source;
        "";
        Sparkline.render_with_label s.cpu_spark ~label:"CPU Usage" ~focus:true 
          ~color:"32" ~thresholds:cpu_thresholds ();
        Sparkline.render_with_label s.mem_spark ~label:"Memory   " ~focus:false 
          ~color:"34" ();
        Sparkline.render_with_label s.net_spark ~label:"Network  " ~focus:false ();
        "";
        sep;
        "";
        W.dim 
  ...
  let handle_key s key_str ~size:_ = 
    ...
    | Some (Miaou.Core.Keys.Char k) when String.lowercase_ascii k = "t" -> 
        show_tutorial_modal ~title:"Sparkline tutorial" ~markdown:tutorial_markdown ;
        s
...
|}
  ...
  let init () = 
    ...
    {
      chart = 
        Line_chart.create ~width:70 ~height:18 
  ...
  let add_points s = 
    ...
    let chart = 
      s.chart 
      |> Line_chart.update_series ~label:"Sine" 
           ~points:(generate_sine_points new_count)
      |> Line_chart.update_series ~label:"Cosine" 
  ...
  let handle_key s key_str ~size:_ = 
    ...
    | Some (Miaou.Core.Keys.Char "Esc") | Some (Miaou.Core.Keys.Char "Escape")
      -> 
        go_home s
    | Some (Miaou.Core.Keys.Char k) when String.lowercase_ascii k = "t" -> 
        show_tutorial_modal ~title:"Line chart tutorial" 
          ~markdown:tutorial_markdown ;
        s
...
module System_monitor_demo_page : Miaou.Core.Tui_page.PAGE_SIG = struct
  ...
  let init () = 
    {
      cpu_spark =
        Sparkline.create ~width:35 ~max_points:35 ~min_value:0.0 
          ~max_value:100.0 ();
      mem_spark =
        Sparkline.create ~width:35 ~max_points:35 ~min_value:0.0 
          ~max_value:100.0 ();
      net_spark =
        Sparkline.create ~width:35 ~max_points:35 ();
      cpu_history = [];
      tick_count = 0;
      next_page = None;
    }
  ...
  let view s ~focus:_ ~size = 
    ...
    let load1, load5, load15 = Miaou_example.System_metrics.get_load_average () in 
    let sys_info = 
      Desc_list.create 
    ...
    let separator_width = 3 in (* "│  " - no space before, two after *)
    ...
    let sparkline_width = max 20 (right_width - 25) in (* Reserve 25 chars for "NET: " + " 100.4 KB/s" *)
    let s_cpu_adjusted = Sparkline.create ~width:sparkline_width ~max_points:sparkline_width 
      ~min_value:0.0 ~max_value:100.0 () in
    let s_mem_adjusted = Sparkline.create ~width:sparkline_width ~max_points:sparkline_width 
      ~min_value:0.0 ~max_value:100.0 () in
    let s_net_adjusted = Sparkline.create ~width:sparkline_width ~max_points:sparkline_width () in
    ...
    let cpu_line_adj = 
      Printf.sprintf "CPU: %5.1f " cpu_val ^ SDL_render.render_sparkline_sdl s_cpu_adjusted ~color:"32" ~thresholds:cpu_thresholds
    in 
    let mem_line_adj = 
      Printf.sprintf "MEM: %5.1f " mem_val ^ SDL_render.render_sparkline_sdl s_mem_adjusted ~color:"34" ~thresholds:[]
    in 
    let net_line_adj = 
      Printf.sprintf "NET: %5.1f KB/s " net_val ^ SDL_render.render_sparkline_sdl s_net_adjusted ~color:"35" ~thresholds:[]
    in
    
    let sys_info_lines = String.split_on_char '\n' (Desc_list.render ~cols:left_width ~wrap:false sys_info ~focus:false) in
    let metrics_title_line = "  " ^ W.fg 45 "★" ^ " " ^ W.fg 213 (W.bold "Real-Time Metrics") in
    let metrics_lines = [metrics_title_line; ""; cpu_line_adj; mem_line_adj; net_line_adj] in
    
    let combined_info = 
      let max_lines = max (List.length sys_info_lines) (List.length metrics_lines) in
      let pad_list lst len = lst @ List.init (len - List.length lst) (fun _ -> "") in
      ...
      List.mapi (fun i (left, right) -> 
        ...
        let stripped = Str.global_replace (Str.regexp "\027\\[[0-9;]*m") "" left in
        let visible_len = String.length stripped in
        (* Title line (i=0) needs MORE padding *)
        let padding = max 0 (if i = 0 then left_width - visible_len + 2 else left_width - visible_len) in
    ...
    let cpu_chart = 
      if List.length s.cpu_history >= 2 then 
        ...
        in 
        let chart = 
          Line_chart.create ~width:(min 80 width) ~height:8 
            ~series:[series] 
    ...
    let hint = 
      W.dim "Auto-updating every ~150ms • Esc to return"
    in 
...
|}
  ...
  let view s ~focus:_ ~size:_ = 
    ...
    in 
    let chart = 
      Bar_chart.create ~width:70 ~height:15 ~data:s.data 
        ~title:"Daily Sales ($)" ()
    in 
    let chart_output = Bar_chart.render chart ~show_values:true ~thresholds () in 
    let hint = 
      W.dim "Space to randomize • t for tutorial • Esc to return"
    in 
    String.concat "\n" [header; ""; chart_output; ""; hint]
  ...
  let handle_key s key_str ~size:_ = 
    ...
    | Some (Miaou.Core.Keys.Char "Esc") | Some (Miaou.Core.Keys.Char "Escape")
      -> 
        go_home s
    | Some (Miaou.Core.Keys.Char k) when String.lowercase_ascii k = "t" -> 
        show_tutorial_modal ~title:"Bar chart tutorial" 
          ~markdown:tutorial_markdown ;
        s
...
module rec Page : Miaou.Core.Tui_page.PAGE_SIG = struct
  ...
  let open_demo s idx = match List.nth_opt demos idx with Some d -> d.open_demo s | None -> s

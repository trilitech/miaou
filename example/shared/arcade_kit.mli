...
module Particles : sig
  ...
  val iter :
    t ->
    f:(x:float -> y:float -> life01:float -> hue:int -> unit) ->
    unit
...
module Score_store : sig
  ...
  (** Write [score] to disk if it improves the previous high score. Silent
      on any IO failure — high scores are not critical-path. *)
...
module Pixel_mode : sig
  val resolve :
    ?env_var:string ->
    unit ->
    Miaou_widgets_display.Terminal_caps.render_mode

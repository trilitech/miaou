#!/usr/bin/env bash
# G3.1/G3.3 narrow-terminal scenario for the term (lambda-term) driver: at
# 60x24 (< 80 cols) the launcher must show both the persistent narrow
# warning banner and the one-time "Narrow terminal" modal; any key
# dismisses the modal while the banner keeps rendering every frame; the
# app still quits cleanly afterwards. Paired with scenario_narrow_matrix.sh
# so the shared Narrow_warning helper (G3.3) is gated on both drivers.
set -u
...
SESSION="miaou_narrow_term_$$"
tmux_new_session "$SESSION" 60 24 sh -c "MIAOU_DRIVER=term $BIN"

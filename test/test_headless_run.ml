...
module Page = struct
  ...
  let handle_key st key ~size:_ = if key = "q" then st else st + String.length key
...
let test_run_loop () =
  ...
  check bool "quit or switch" true (match res with `Quit | `SwitchTo _ -> true)

let () = run "headless_run" [("headless_run", [test_case "run loop" `Quick test_run_loop])]

...
let icon_for ~name ~is_dir =
  ...
  let icon =
    if name = ".." then if nerd then parent_nerd else parent_unicode
    else if is_dir then if nerd then dir_nerd else dir_unicode
    else
      let ext = extension_lower name in
      match lookup ext with
      | Some (u, n, _) -> if nerd then n else u
      | None -> if nerd then plain_nerd else plain_unicode
  in
  icon ^ " "

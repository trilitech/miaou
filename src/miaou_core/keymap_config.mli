...
(** User-supplied keybinding overrides.
    ...
    Page-specific rules take precedence over global [page=*] fallback rules.
    Within each tier, rules are matched in declaration order and the first
    rule whose [key] matches the pressed key wins. Any line beginning with [#]
    (after leading whitespace) is treated as a comment, blank lines are
    ignored.
...
(** Find the user-defined action for a [(page, key)] pair, if any.
    [page]-specific rules are searched first, followed by global ([page=*])
    rules as fallbacks. *)

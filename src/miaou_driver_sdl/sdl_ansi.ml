...
let ansi_bright_palette =
  [|
    {Sdl_colors.r = 102; g = 102; b = 102; a = 255};
...
let color256 idx =
  ...
  if idx < 16 then
    ...
    {Sdl_colors.r = to_int r; g = to_int g; b = to_int b; a = 255}
  else
    let level = 8 + ((idx - 232) * 10) in
    {Sdl_colors.r = level; g = level; b = level; a = 255}
...
let parse_ansi_segments ~(default : ansi_state) (s : string) =
  ...
  let add_chunk acc (state : ansi_state) =
    if Buffer.length buf = 0 then acc
    else
      let chunk = Buffer.contents buf in
      Buffer.clear buf ;
      {text = chunk; fg = state.fg; bg = state.bg} :: acc
  in
  let rec loop i acc (state : ansi_state) =

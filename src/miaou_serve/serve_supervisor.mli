...
(** [run ?auth_token ?auth_file ?port ?bind ?max_sessions ?idle_timeout
    ?insecure_allow_plaintext_external page] is the Slice 2 supervisor
    entry point: enforces the fail-closed bind policy (FR-003), creates
    the [0700] socket directory, spawns exactly one worker (Slice 2 is
    single-session; Slice 3 generalizes to a session table), waits for it
    to become reachable (bounded retry), generates a controller-role
    session token (FR-030), prints the [/s/<token>/] session URL, and
    serves the public TCP listener as a byte proxy to that worker
    ({!Serve_proxy.handle_connection}) until the process is signaled to
    stop. [max_sessions]/[idle_timeout] are accepted and recorded but not
    yet enforced (Slice 4). *)
val run :
  ?auth_token:string ->
  ?auth_file:string ->
  ?port:int ->

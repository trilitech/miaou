  ...
  while !pos < len do
    ...
    | Normal -> (
        ...
        | _ ->
            let char, char_len = extract_utf8_char input !pos in
            if char_len > 0 then begin
              ops.set_char ~row ~col:!col ~char ~style:t.style ;
              incr col ;
              pos := !pos + char_len
            end
            else incr pos)
